...
import { templateSpec } from '../Specifications/TemplateSpec';
...
export function compile(jsEditor, jsonEditor) {
	...
	const jsonSpec = JSON.parse(JSON.stringify(templateSpec));
		...
		(e) => {
			...
			switch(callee) {
				...
				case "pop":
				{
					...
					jsonSpec.transform.push({type: "remove", index: [data.length - 1]});
				...
				case "splice":
				{
					...
					const endIndex = e.arguments[1] === undefined ? data.length -1 : startIndex + e.arguments[1] - 1;
					const removeIndex = (new Array(endIndex - startIndex + 1).fill(0)).map((item, index) => startIndex + index);

					//Items to be added
					const addValue = e.arguments.slice(2);
					const addIndex = (new Array(addValue.length).fill(0)).map((items, index) => startIndex + index);
				...
				case "reverse":
                {
                    ...
                    const arrCopy = new Array(arrLen).fill(0).map((item, index) => data[data.length - index - 1]);
                    jsonSpec.transform.push(
                        {
                            type: "update",
                            value: arrCopy
