...
export class AnimationSpec {
	...
	constructor(rawSpec) {
		...
		//Global properties
		this.global.stagger =
			this.global.stagger !== undefined ? this.global.stagger : false;
		this.global.easingFunction =
			this.global.easingFunction !== undefined
				? `d3.${this.global.easingFunction}`
				: 'd3.easeCubic';
		this.global.durationEach =
			this.global.durationEach !== undefined
				? this.global.durationEach
				: 1000;
		...
		Object.keys(transformType).forEach((key) => {
			...
			const stagger =
				propExist && rawSpec[rawSpecKey].stagger !== undefined
					? rawSpec[rawSpecKey].stagger
					: this.global.stagger;
			const easingFunction =
				propExist && rawSpec[rawSpecKey].easingFunction !== undefined
					? `d3.${rawSpec[rawSpecKey].easingFunction}`
					: this.global.easingFunction;
			const durationEach =
				propExist && rawSpec[rawSpecKey].durationEach !== undefined
					? rawSpec[rawSpecKey].durationEach
					: this.global.durationEach;
