	...
	if (stagger) {
		...
		for (let i = 0; i < indexesToCombine.length; i++) {
			...
			const endSel = d3.selectAll(
				endIndex.map((item, index, array) => arrayDiagram.items.nodes()[array[index]])
			);
		...
		tempData = tempData.filter((item, index) => !startIndexes.includes(index));

		await updateArrayDiagram(tempData, arrayDiagram, duration, stagger);
	} else {
		const startIndexes = indexesToCombine.map((item, index) => item[0]);
		...
		const endSel = d3.selectAll(
			endIndexes.map((item, index, array) => arrayDiagram.items.nodes()[array[index]])
		);
		...
		const morphPromise = morphArrayElement(endSel, valuesToReplace, duration, stagger);
		const updatePromise = updateArrayDiagram(arrayDiagram, duration, stagger);
