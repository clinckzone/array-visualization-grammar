 ...
 */
export async function returnSelection(arrayDiagram, returnArray, index, duration, stagger) {
	//Map the indexes to nodes in arrayItems of the arrayDiagram
	let selection = d3.selectAll(index.map((item) => arrayDiagram.items.nodes()[item]));

	//Create a copy of the selection
	const copiedNodes = selection.nodes().map((item) => item.cloneNode(true));
	selection = d3.selectAll(
		copiedNodes.map((item) => document.getElementById('svg-container').appendChild(item))
	);

	//Remove the old arrayDiagram class identifier from the item
	selection.classed(`array-item-${arrayDiagram.properties.id}`, false);
	...
	selection.attr('class', `array-item-${returnArray.properties.id}`);
	selection.datum((data, index) => returnArray.data[toIndex[index]]);
	selection.raise();

	await updateArrayDiagram(returnArray.data, returnArray, duration / 4, stagger);
}
